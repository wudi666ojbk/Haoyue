...
namespace Haoyue {
	...
	struct EntityScriptClass
	{
		...
		void InitClassMethods(MonoImage* image)
		{
			...
			OnCollisionEndMethod = GetMethod(s_CoreAssemblyImage, "Haoyue.Entity:OnCollisionEnd(single)");
			OnTriggerBeginMethod = GetMethod(s_CoreAssemblyImage, "Haoyue.Entity:OnTriggerBegin(single)");
			OnTriggerEndMethod = GetMethod(s_CoreAssemblyImage, "Haoyue.Entity:OnTriggerEnd(single)");
			OnCollision2DBeginMethod = GetMethod(s_CoreAssemblyImage, "Haoyue.Entity:OnCollision2DBegin(single)");
			OnCollision2DEndMethod = GetMethod(s_CoreAssemblyImage, "Haoyue.Entity:OnCollision2DEnd(single)");
		}
	};

	MonoObject* EntityInstance::GetInstance()
	{
		HY_CORE_ASSERT(Handle, "Entity has not been instantiated!");
	...
	void ScriptEngine::LoadHaoyueRuntimeAssembly(const std::string& path)
	{
		...
		if (s_MonoDomain)
		{
			domain = mono_domain_create_appdomain("Haoyue Runtime", nullptr);
			mono_domain_set(domain, false);
			
			cleanup = true;
		}

		s_CoreAssembly = LoadAssembly("Resources/scripts/Haoyue-ScriptCore.dll");
	...
	{
		HY_CORE_ASSERT(s_EntityInstanceMap.find(dst) != s_EntityInstanceMap.end());
		HY_CORE_ASSERT(s_EntityInstanceMap.find(src) != s_EntityInstanceMap.end());
		...
		for (auto& [entityID, entityInstanceData] : srcEntityMap)
		{
			for (auto& [moduleName, srcFieldMap] : srcEntityMap[entityID].ModuleFieldMap)
			{
				...
				{
					HY_CORE_ASSERT(dstModuleFieldMap.find(moduleName) != dstModuleFieldMap.end());
					auto& fieldMap = dstModuleFieldMap.at(moduleName);
					HY_CORE_ASSERT(fieldMap.find(fieldName) != fieldMap.end());
	...
	{
		HY_CORE_ASSERT(s_EntityInstanceMap.find(sceneID) != s_EntityInstanceMap.end());
		auto& entityMap = s_EntityInstanceMap.at(sceneID);
		HY_CORE_ASSERT(entityMap.find(entityID) != entityMap.end());
	...
	static FieldType GetHaoyueFieldType(MonoType* monoType)
	{
		...
		switch (type)
		{
			...
			case MONO_TYPE_VALUETYPE:
			{
				...
				if (strcmp(name, "Haoyue.Vector2") == 0) return FieldType::Vec2;
				if (strcmp(name, "Haoyue.Vector3") == 0) return FieldType::Vec3;
				if (strcmp(name, "Haoyue.Vector4") == 0) return FieldType::Vec4;
			}
		}
	...
	void ScriptEngine::InitScriptEntity(Entity entity)
	{
		...
		{
			HY_CORE_ERROR("Entity references non-existent script module '{0}'", moduleName);
			return;
		}
	...
	void ScriptEngine::InstantiateEntityClass(Entity entity)
	{
		...
		EntityInstance& entityInstance = entityInstanceData.Instance;
		HY_CORE_ASSERT(entityInstance.ScriptClass);
	...
	{
		HY_CORE_ASSERT(s_EntityInstanceMap.find(sceneID) != s_EntityInstanceMap.end(), "Invalid scene ID!");
		auto& entityIDMap = s_EntityInstanceMap.at(sceneID);
		HY_CORE_ASSERT(entityIDMap.find(entityID) != entityIDMap.end(), "Invalid entity ID!");
	...
	static uint32_t GetFieldSize(FieldType type)
	{
		...
		HY_CORE_ASSERT(false, "Unknown field type!");
		return 0;
	}
	...
	{
		HY_CORE_ASSERT(m_EntityInstance->GetInstance());
