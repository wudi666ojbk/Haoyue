...
#include "Haoyue/Core/Math/Noise.h"

#include "Haoyue/Scene/Scene.h"
#include "Haoyue/Scene/Entity.h"
#include "Haoyue/Physics/PhysicsUtil.h"
...
namespace Haoyue { namespace Script {
	...
	static void AddCollidersToArray(MonoArray* array, const std::array<physx::PxOverlapHit, OVERLAP_MAX_COLLIDERS>& hits, uint32_t count, uint32_t arrayLength)
	{
		...
		for (uint32_t i = 0; i < count; i++)
		{
			...
			if (entity.HasComponent<BoxColliderComponent>() && arrayIndex < arrayLength)
			{
				...
				MonoObject* obj = ScriptEngine::Construct("Haoyue.BoxCollider:.ctor(ulong,bool,Vector3,Vector3)", true, data);
			...
			if (entity.HasComponent<SphereColliderComponent>() && arrayIndex < arrayLength)
			{
				...
				MonoObject* obj = ScriptEngine::Construct("Haoyue.SphereCollider:.ctor(ulong,bool,single)", true, data);
				mono_array_set(array, MonoObject*, arrayIndex++, obj);
			}
			...
			if (entity.HasComponent<CapsuleColliderComponent>() && arrayIndex < arrayLength)
			{
				...
				MonoObject* obj = ScriptEngine::Construct("Haoyue.CapsuleCollider:.ctor(ulong,bool,single,single)", true, data);
				mono_array_set(array, MonoObject*, arrayIndex++, obj);
			}
			...
			if (entity.HasComponent<MeshColliderComponent>() && arrayIndex < arrayLength)
			{
				...
				MonoObject* obj = ScriptEngine::Construct("Haoyue.MeshCollider:.ctor(ulong,bool,intptr)", true, data);
				mono_array_set(array, MonoObject*, arrayIndex++, obj);
			}
		}
	}
	...
	MonoArray* Haoyue_Physics_OverlapBox(glm::vec3* origin, glm::vec3* halfSize)
	{
		...
		{
			outColliders = mono_array_new(mono_domain_get(), ScriptEngine::GetCoreClass("Haoyue.Collider"), count);
	...
	void Haoyue_RigidBody2DComponent_ApplyLinearImpulse(uint64_t entityID, glm::vec2* impulse, glm::vec2* offset, bool wake)
	{
		...
		Entity entity = entityMap.at(entityID);
		HY_CORE_ASSERT(entity.HasComponent<RigidBody2DComponent>());
	...
	RigidBodyComponent::Type Haoyue_RigidBodyComponent_GetBodyType(uint64_t entityID)
	{
		...
		Entity entity = entityMap.at(entityID);
		HY_CORE_ASSERT(entity.HasComponent<RigidBodyComponent>());
	...
	void Haoyue_RigidBodyComponent_AddForce(uint64_t entityID, glm::vec3* force, ForceMode forceMode)
	{
		...
		{
			HY_CORE_WARN("Cannot add a force to a kinematic actor! EntityID({0})", entityID);
			return;
		}
	...
	void Haoyue_RigidBodyComponent_AddTorque(uint64_t entityID, glm::vec3* torque, ForceMode forceMode)
	{
		...
		if (component.IsKinematic)
		{
			HY_CORE_WARN("Cannot add torque to a kinematic actor! EntityID({0})", entityID);
			return;
		}
	...
	Ref<Material>* Haoyue_Mesh_GetMaterialByIndex(Ref<Mesh>* inMesh, int index)
	{
		...
		HY_CORE_ASSERT(index < materials.size());
	...
	void Haoyue_Texture2D_SetData(Ref<Texture2D>* _this, MonoArray* inData, int32_t count)
	{
		...
		HY_CORE_ASSERT(dataSize <= buffer.Size);
