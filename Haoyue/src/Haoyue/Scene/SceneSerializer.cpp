...
namespace Haoyue {
	...
	bool SceneSerializer::Deserialize(const std::string& filepath)
	{
		...
		HY_CORE_INFO("Deserializing scene '{0}'", sceneName);
		...
		if (entities)
		{
			for (auto entity : entities)
			{
				...
				HY_CORE_INFO("Deserialized entity with ID = {0}, name = {1}", uuid, name);
				...
				if (transformComponent)
				{
					...
					{
						HY_CORE_ASSERT(rotationNode.size() == 3);
						transform.Rotation = transformComponent["Rotation"].as<glm::vec3>();
					}
					transform.Scale = transformComponent["Scale"].as<glm::vec3>();

					HY_CORE_INFO("  Entity Transform:");
					HY_CORE_INFO("    Translation: {0}, {1}, {2}", transform.Translation.x, transform.Translation.y, transform.Translation.z);
					HY_CORE_INFO("    Rotation: {0}, {1}, {2}", transform.Rotation.x, transform.Rotation.y, transform.Rotation.z);
					HY_CORE_INFO("    Scale: {0}, {1}, {2}", transform.Scale.x, transform.Scale.y, transform.Scale.z);
				}
				...
				if (scriptComponent)
				{
					...
					HY_CORE_INFO("  Script Module: {0}", moduleName);
				...
				if (skyLightComponent)
				{
					...
					else
					{
						...
						if (filepath.empty())
							HY_CORE_ERROR("Tried to load non-existent environment map in Entity: {0}", deserializedEntity.GetUUID());
						else
							HY_CORE_ERROR("Tried to load invalid environment map '{0}' in Entity {1}", filepath, deserializedEntity.GetUUID());
					}
				...
				if (boxColliderComponent)
				{
					...
					if (material)
					{
						...
						{
							HY_CORE_ERROR("Tried to load invalid Physics Material in Entity {0}", deserializedEntity.GetUUID());
						}
					}
				...
				if (meshColliderComponent)
				{
					...
					{
						HY_CORE_WARN("MeshColliderComponent in use without valid mesh!");
					}
