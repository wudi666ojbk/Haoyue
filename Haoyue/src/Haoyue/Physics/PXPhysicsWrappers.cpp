...
namespace Haoyue {
	...
	void PhysicsErrorCallback::reportError(physx::PxErrorCode::Enum code, const char* message, const char* file, int line)
	{
		...
		case physx::PxErrorCode::eDEBUG_INFO:
			HY_CORE_INFO("[PhysX]: {0}: {1} at {2} ({3})", errorMessage, message, file, line);
			break;
		case physx::PxErrorCode::eDEBUG_WARNING:
		case physx::PxErrorCode::ePERF_WARNING:
			HY_CORE_WARN("[PhysX]: {0}: {1} at {2} ({3})", errorMessage, message, file, line);
			break;
		case physx::PxErrorCode::eINVALID_PARAMETER:
		case physx::PxErrorCode::eINVALID_OPERATION:
		case physx::PxErrorCode::eOUT_OF_MEMORY:
		case physx::PxErrorCode::eINTERNAL_ERROR:
			HY_CORE_ERROR("[PhysX]: {0}: {1} at {2} ({3})", errorMessage, message, file, line);
			break;
		case physx::PxErrorCode::eABORT:
		case physx::PxErrorCode::eMASK_ALL:
			HY_CORE_FATAL("[PhysX]: {0}: {1} at {2} ({3})", errorMessage, message, file, line);
	...
	void ContactListener3D::onWake(physx::PxActor** actors, physx::PxU32 count)
	{
		for (uint32_t i = 0; i < count; i++)
		{
			...
			HY_CORE_INFO("PhysX Actor waking up: ID: {0}, Name: {1}", entity.GetUUID(), entity.GetComponent<TagComponent>().Tag);
		}
	}
	...
	void ContactListener3D::onSleep(physx::PxActor** actors, physx::PxU32 count)
	{
		for (uint32_t i = 0; i < count; i++)
		{
			...
			HY_CORE_INFO("PhysX Actor going to sleep: ID: {0}, Name: {1}", entity.GetUUID(), entity.GetComponent<TagComponent>().Tag);
		}
	}
	...
	static physx::PxBroadPhaseType::Enum HaoyueToPhysXBroadphaseType(BroadphaseType type)
	{
		switch (type)
		{
		case Haoyue::BroadphaseType::SweepAndPrune: return physx::PxBroadPhaseType::eSAP;
		case Haoyue::BroadphaseType::MultiBoxPrune: return physx::PxBroadPhaseType::eMBP;
		case Haoyue::BroadphaseType::AutomaticBoxPrune: return physx::PxBroadPhaseType::eABP;
		}
	...
	static physx::PxFrictionType::Enum HaoyueToPhysXFrictionType(FrictionType type)
	{
		switch (type)
		{
		case Haoyue::FrictionType::Patch:			return physx::PxFrictionType::ePATCH;
		case Haoyue::FrictionType::OneDirectional:	return physx::PxFrictionType::eONE_DIRECTIONAL;
		case Haoyue::FrictionType::TwoDirectional:	return physx::PxFrictionType::eTWO_DIRECTIONAL;
		}
	...
	std::vector<physx::PxShape*> PXPhysicsWrappers::CreateConvexMesh(MeshColliderComponent& collider, const glm::vec3& scale, bool invalidateOld)
	{
		...
		if (!PhysicsMeshSerializer::IsSerialized(collisionMeshAsset->GetFilePath()))
		{
			...
			for (uint32_t submeshIndex : submeshes)
			{
				...
				{
					HY_CORE_ERROR("Failed to cook convex mesh {0}", submesh.MeshName);
					continue;
				}
	...
	std::vector<physx::PxShape*> PXPhysicsWrappers::CreateTriangleMesh(MeshColliderComponent& collider, const glm::vec3& scale, bool invalidateOld)
	{
		...
		if (!PhysicsMeshSerializer::IsSerialized(collisionMeshAsset->GetFilePath()))
		{
			...
			for (uint32_t submeshIndex : submeshes)
			{
				...
				{
					HY_CORE_ERROR("Failed to cook triangle mesh: {0}", submesh.MeshName);
					continue;
				}
	...
	{
		HY_CORE_ASSERT(!s_Foundation, "PXPhysicsWrappers::Initializer shouldn't be called more than once!");

		s_Foundation = PxCreateFoundation(PX_PHYSICS_VERSION, s_Allocator, s_ErrorCallback);
		HY_CORE_ASSERT(s_Foundation, "PxCreateFoundation Failed!");
		...
		HY_CORE_ASSERT(s_Physics, "PxCreatePhysics Failed!");

		s_CookingFactory = PxCreateCooking(PX_PHYSICS_VERSION, *s_Foundation, s_Physics->getTolerancesScale());
		HY_CORE_ASSERT(s_CookingFactory, "PxCreatePhysics Failed!");
	...
	{
		HY_CORE_ERROR("[PhysX Error]: {0}:{1} - {2}", file, line, exp);
	}
