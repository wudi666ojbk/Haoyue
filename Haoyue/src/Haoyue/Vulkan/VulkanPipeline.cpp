...
namespace Haoyue {
	...
	namespace Utils
	{
		static VkPrimitiveTopology GetVulkanTopology(PrimitiveTopology topology)
		{
			...
			HY_CORE_ASSERT(false, "Unknown toplogy");
	...
	{
		HY_CORE_ASSERT(spec.Shader);
		HY_CORE_ASSERT(spec.RenderPass);
		Invalidate();
	...
	void VulkanPipeline::Invalidate()
	{
		...
		Renderer::Submit([instance]() mutable
			{
				HY_CORE_WARN("[VulkanPipeline] Creating pipeline {0}", instance->m_Specification.DebugName);

				VkDevice device = VulkanContext::GetCurrentDevice()->GetVulkanDevice();
				HY_CORE_ASSERT(instance->m_Specification.Shader);
