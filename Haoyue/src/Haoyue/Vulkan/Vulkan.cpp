	...
	void RetrieveDiagnosticCheckpoints()
	{
...

		{
			...
			for (uint32_t i = 0; i < retrievedCount; i++)
			{
				...
				HY_CORE_ERROR("Checkpoint: {0} (stage: {1})", checkpoint->Data, StageToString(data[i].stage));
			}
		}
		{
			const uint32_t checkpointCount = 4;
			...
			uint32_t retrievedCount = checkpointCount;
			vkGetQueueCheckpointDataNV(::Haoyue::VulkanContext::GetCurrentDevice()->GetComputeQueue(), &retrievedCount, data);
			HY_CORE_ERROR("RetrieveDiagnosticCheckpoints (Compute Queue):");
			for (uint32_t i = 0; i < retrievedCount; i++)
			{
				VulkanCheckpointData* checkpoint = (VulkanCheckpointData*)data[i].pCheckpointMarker;
				HY_CORE_ERROR("Checkpoint: {0} (stage: {1})", checkpoint->Data, StageToString(data[i].stage));
