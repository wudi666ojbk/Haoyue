...
namespace Haoyue {
	...
	public:
		...
		void Set(const std::string& name, const T& value)
		{
			...
			HY_CORE_ASSERT(decl, "Could not find uniform!");
			if (!decl)
				return;
		...
		{
			auto decl = FindUniformDeclaration(name);
			HY_CORE_ASSERT(decl, "Could not find uniform with name 'x'");
			auto& buffer = m_UniformStorageBuffer;
		...
		Ref<T> GetResource(const std::string& name)
		{
			...
			HY_CORE_ASSERT(slot < m_Textures.size(), "Texture slot is invalid!");
