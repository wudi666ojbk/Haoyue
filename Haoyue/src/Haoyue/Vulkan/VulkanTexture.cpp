...
namespace Haoyue {
	...
	namespace Utils {
		...
		static VkSamplerAddressMode VulkanSamplerWrap(TextureWrap wrap)
		{
			...
			HY_CORE_ASSERT(false, "Unknown wrap mode");
		...
		static VkFilter VulkanSamplerFilter(TextureFilter filter)
		{
			...
			HY_CORE_ASSERT(false, "Unknown filter");
			return (VkFilter)0;
		}
		...
		: m_Path(path), m_Properties(properties)
	{
		...
		HY_CORE_ASSERT(m_Format != ImageFormat::None);
		...
		: m_Format(format), m_Properties(properties)
	{
		...
		HY_CORE_ASSERT(format == ImageFormat::RGBA);
	...
	{
		HY_CORE_ASSERT(false, "Not implemented");
	}
