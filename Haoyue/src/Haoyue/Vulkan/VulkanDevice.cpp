...
namespace Haoyue {
	...
	VulkanPhysicalDevice::VulkanPhysicalDevice()
	{
		...
		{
			HY_CORE_TRACE("Could not find discrete GPU.");
			selectedPhysicalDevice = physicalDevices.back();
		}

		HY_CORE_ASSERT(selectedPhysicalDevice, "Could not find any physical devices!");
		...
		HY_CORE_ASSERT(queueFamilyCount > 0, "");
		...
		if (extCount > 0)
		{
			...
			{
				HY_CORE_TRACE("Selected physical device has {0} extensions", extensions.size());
				for (const auto& ext : extensions)
				{
					m_SupportedExtensions.emplace(ext.extensionName);
					HY_CORE_TRACE("  {0}", ext.extensionName);
				}
			}
		}
	...
	uint32_t VulkanPhysicalDevice::GetMemoryTypeIndex(uint32_t typeBits, VkMemoryPropertyFlags properties) const
	{
		...
		HY_CORE_ASSERT(false, "Could not find a suitable memory type!");
		return UINT32_MAX;
	}
		...
		: m_PhysicalDevice(physicalDevice), m_EnabledFeatures(enabledFeatures)
	{
		...
		HY_CORE_ASSERT(m_PhysicalDevice->IsExtensionSupported(VK_KHR_SWAPCHAIN_EXTENSION_NAME));
		...
		HY_CORE_ASSERT(result == VK_SUCCESS);
	...
	void VulkanDevice::FlushCommandBuffer(VkCommandBuffer commandBuffer, VkQueue queue)
	{
		...
		HY_CORE_ASSERT(commandBuffer != VK_NULL_HANDLE);
