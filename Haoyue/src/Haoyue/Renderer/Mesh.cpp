...
#if MESH_DEBUG_LOG
#define HY_MESH_LOG(...) HY_CORE_TRACE(__VA_ARGS__)
#else
#define HY_MESH_LOG(...)
#endif
	...
	struct LogStream : public Assimp::LogStream
	{
		...
		{
			HY_CORE_ERROR("Assimp error: {0}", message);
		}
	};
		...
		: m_FilePath(filename)
	{
		...
		if (!scene || !scene->HasMeshes())
			HY_CORE_ERROR("Failed to load mesh file: {0}", filename);
		...
		for (size_t m = 0; m < scene->mNumMeshes; m++)
		{
			...
			HY_CORE_ASSERT(mesh->HasPositions(), "Meshes require positions.");
			HY_CORE_ASSERT(mesh->HasNormals(), "Meshes require normals.");
			...
			{
				HY_CORE_ASSERT(mesh->mFaces[i].mNumIndices == 3, "Must have 3 indices.");
		...
		if (m_IsAnimated)
		{
			for (size_t m = 0; m < scene->mNumMeshes; m++)
			{
				...
				for (size_t i = 0; i < mesh->mNumBones; i++)
				{
					...
					{
						HY_MESH_LOG("Found existing bone in map");
		...
		{
			HY_MESH_LOG("---- Materials - {0} ----", filename);
			...
			for (uint32_t i = 0; i < scene->mNumMaterials; i++)
			{
				...
				HY_MESH_LOG("  {0} (Index = {1})", aiMaterialName.data, i);
				aiString aiTexPath;
				...
				HY_MESH_LOG("    COLOR = {0}, {1}, {2}", aiColor.r, aiColor.g, aiColor.b);
				HY_MESH_LOG("    ROUGHNESS = {0}", roughness);
				HY_MESH_LOG("    METALNESS = {0}", metalness);
				bool hasAlbedoMap = aiMaterial->GetTexture(aiTextureType_DIFFUSE, 0, &aiTexPath) == AI_SUCCESS;
				bool fallback = !hasAlbedoMap;
				if (hasAlbedoMap)
				{
					// TODO: Temp - this should be handled by Haoyue's filesystem
					std::filesystem::path path = filename;
					auto parentPath = path.parent_path();
					parentPath /= std::string(aiTexPath.data);
					std::string texturePath = parentPath.string();
					HY_MESH_LOG("    Albedo map path = {0}", texturePath);
					...
					{
						HY_CORE_ERROR("Could not load texture: {0}", texturePath);
						fallback = true;
					}
				}
				...
				if (hasNormalMap)
				{
					...
					parentPath /= std::string(aiTexPath.data);
					std::string texturePath = parentPath.string();
					HY_MESH_LOG("    Normal map path = {0}", texturePath);
				...
				if (hasRoughnessMap)
				{
					...
					parentPath /= std::string(aiTexPath.data);
					std::string texturePath = parentPath.string();
					HY_MESH_LOG("    Roughness map path = {0}", texturePath);
					auto texture = Texture2D::Create(texturePath);
					if (texture->Loaded())
					{
						m_Textures.push_back(texture);
				...
				for (uint32_t p = 0; p < aiMaterial->mNumProperties; p++)
				{
					...
					HY_MESH_LOG("Material Property:");
					HY_MESH_LOG("  Name = {0}", prop->mKey.data);
					// HY_MESH_LOG("  Type = {0}", prop->mType);
					// HY_MESH_LOG("  Size = {0}", prop->mDataLength);
					float data = *(float*)prop->mData;
					HY_MESH_LOG("  Value = {0}", data);

					switch (prop->mSemantic)
					{
					case aiTextureType_NONE:
						HY_MESH_LOG("  Semantic = aiTextureType_NONE");
						break;
					case aiTextureType_DIFFUSE:
						HY_MESH_LOG("  Semantic = aiTextureType_DIFFUSE");
						break;
					case aiTextureType_SPECULAR:
						HY_MESH_LOG("  Semantic = aiTextureType_SPECULAR");
						break;
					case aiTextureType_AMBIENT:
						HY_MESH_LOG("  Semantic = aiTextureType_AMBIENT");
						break;
					case aiTextureType_EMISSIVE:
						HY_MESH_LOG("  Semantic = aiTextureType_EMISSIVE");
						break;
					case aiTextureType_HEIGHT:
						HY_MESH_LOG("  Semantic = aiTextureType_HEIGHT");
						break;
					case aiTextureType_NORMALS:
						HY_MESH_LOG("  Semantic = aiTextureType_NORMALS");
						break;
					case aiTextureType_SHININESS:
						HY_MESH_LOG("  Semantic = aiTextureType_SHININESS");
						break;
					case aiTextureType_OPACITY:
						HY_MESH_LOG("  Semantic = aiTextureType_OPACITY");
						break;
					case aiTextureType_DISPLACEMENT:
						HY_MESH_LOG("  Semantic = aiTextureType_DISPLACEMENT");
						break;
					case aiTextureType_LIGHTMAP:
						HY_MESH_LOG("  Semantic = aiTextureType_LIGHTMAP");
						break;
					case aiTextureType_REFLECTION:
						HY_MESH_LOG("  Semantic = aiTextureType_REFLECTION");
						break;
					case aiTextureType_UNKNOWN:
						HY_MESH_LOG("  Semantic = aiTextureType_UNKNOWN");
						break;
					}
#endif
					...
					if (prop->mType == aiPTI_String)
					{
						...
						if (key == "$raw.ReflectionFactor|file")
						{
							...
							parentPath /= str;
							std::string texturePath = parentPath.string();
							HY_MESH_LOG("    Metalness map path = {0}", texturePath);
							auto texture = Texture2D::Create(texturePath);
							if (texture->Loaded())
							{
		...
		{
			auto mi = Material::Create(m_MeshShader, "Haoyue-Default");
	...
	void Mesh::TraverseNodes(aiNode* node, const glm::mat4& parentTransform, uint32_t level)
	{
		...
		// HY_MESH_LOG("{0} {1}", LevelToSpaces(level), node->mName.C_Str());
	...
	glm::vec3 Mesh::InterpolateTranslation(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		HY_CORE_ASSERT(NextPositionIndex < nodeAnim->mNumPositionKeys);
		float DeltaTime = (float)(nodeAnim->mPositionKeys[NextPositionIndex].mTime - nodeAnim->mPositionKeys[PositionIndex].mTime);
		float Factor = (animationTime - (float)nodeAnim->mPositionKeys[PositionIndex].mTime) / DeltaTime;
		HY_CORE_ASSERT(Factor <= 1.0f, "Factor must be below 1.0f");
	...
	glm::quat Mesh::InterpolateRotation(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		HY_CORE_ASSERT(NextRotationIndex < nodeAnim->mNumRotationKeys);
	...
	glm::vec3 Mesh::InterpolateScale(float animationTime, const aiNodeAnim* nodeAnim)
	{
		...
		HY_CORE_ASSERT(nextIndex < nodeAnim->mNumScalingKeys);
	...
	void Mesh::DumpVertexBuffer()
	{
		...
		HY_MESH_LOG("------------------------------------------------------");
		HY_MESH_LOG("Vertex Buffer Dump");
		HY_MESH_LOG("Mesh: {0}", m_FilePath);
		if (m_IsAnimated)
		{
			for (size_t i = 0; i < m_AnimatedVertices.size(); i++)
			{
				auto& vertex = m_AnimatedVertices[i];
				HY_MESH_LOG("Vertex: {0}", i);
				HY_MESH_LOG("Position: {0}, {1}, {2}", vertex.Position.x, vertex.Position.y, vertex.Position.z);
				HY_MESH_LOG("Normal: {0}, {1}, {2}", vertex.Normal.x, vertex.Normal.y, vertex.Normal.z);
				HY_MESH_LOG("Binormal: {0}, {1}, {2}", vertex.Binormal.x, vertex.Binormal.y, vertex.Binormal.z);
				HY_MESH_LOG("Tangent: {0}, {1}, {2}", vertex.Tangent.x, vertex.Tangent.y, vertex.Tangent.z);
				HY_MESH_LOG("TexCoord: {0}, {1}", vertex.Texcoord.x, vertex.Texcoord.y);
				HY_MESH_LOG("--");
			}
		}
		else
