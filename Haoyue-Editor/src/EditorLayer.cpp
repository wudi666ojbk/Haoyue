...
namespace Haoyue {
	...
	void EditorLayer::UpdateWindowTitle(const std::string& sceneName)
	{
		...
		std::string title = sceneName + " - Haoyue-Editor - " + Application::GetPlatformName() + " (" + Application::GetConfigurationName() + ") Renderer: " + rendererAPI;
	...
	void EditorLayer::OpenScene()
	{
		...
		std::string filepath = app.OpenFile("Haoyue Scene (*.hsc)\0*.hsc\0");
		if (!filepath.empty())
			OpenScene(filepath);
	}
	...
	{
		auto& app = Application::Get();
		std::string filepath = app.SaveFile("Haoyue Scene (*.hsc)\0*.hsc\0");
		if (!filepath.empty())
		{
			SceneSerializer serializer(m_EditorScene);
	...
	void EditorLayer::OnImGuiRender()
	{
		...
		ImGui::Begin("##tool_bar", NULL, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse);
		{
			...
			Ref<Texture2D> buttonTex = m_SceneState == SceneState::Play ? EditorResources::StopButtonTex : EditorResources::PlayIcon;
			...
			if (UI::ImageButton(EditorResources::PauseButtonIcon, ImVec2(size, size), ImVec2(0, 0), ImVec2(1, 1), 0))
			{
		...
		if (m_GizmoType != -1 && m_SelectionContext.size())
		{
			...
			bool snap = Input::IsKeyPressed(HY_KEY_LEFT_CONTROL);
					...
					if (selectedMaterialIndex < materials.size())
					{
						...
						// Textures ------------------------------------------------------------------------------
						{
							...
							if (ImGui::CollapsingHeader("Albedo", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
							{
								...
								Ref<Texture2D> albedoUITexture = hasAlbedoMap ? albedoMap : EditorResources::CheckerboardTex;
						...
						}
						{
							...
							if (ImGui::CollapsingHeader("Normals", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
							{
								...
								UI::Image((normalMap && normalMap->GetImage()) ? normalMap : EditorResources::CheckerboardTex, ImVec2(64, 64));
						...
						}
						{
							...
							if (ImGui::CollapsingHeader("Metalness", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
							{
								...
								UI::Image((metalnessMap && metalnessMap->GetImage()) ? metalnessMap : EditorResources::CheckerboardTex, ImVec2(64, 64));
						...
						}
						{
							...
							if (ImGui::CollapsingHeader("Roughness", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
							{
								...
								UI::Image((roughnessMap && roughnessMap->GetImage()) ? roughnessMap : EditorResources::CheckerboardTex, ImVec2(64, 64));
	...
	bool EditorLayer::OnKeyPressedEvent(KeyPressedEvent& e)
	{
		...
		if (Input::IsKeyPressed(HY_KEY_LEFT_CONTROL))
		{
			switch (e.GetKeyCode())
			{
				case KeyCode::B:
	...
	bool EditorLayer::OnMouseButtonPressed(MouseButtonPressedEvent& e)
	{
		...
		if (e.GetMouseButton() == HY_MOUSE_BUTTON_LEFT && m_ViewportPanelMouseOver && !Input::IsKeyPressed(KeyCode::LeftAlt) && !ImGuizmo::IsOver() && m_SceneState != SceneState::Play)
		{
					...
					for (uint32_t i = 0; i < submeshes.size(); i++)
					{
						...
						if (intersects)
						{
							...
							for (const auto& triangle : triangleCache)
							{
								...
								{
									HY_WARN("INTERSECTION: {0}, t={1}", submesh.NodeName, t);
